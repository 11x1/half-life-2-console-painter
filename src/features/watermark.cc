...
void hl2_watermark::run( ) {
    static constexpr auto watermark_fmt = L"hl2 internal (oops) | {} | battery: {} | b: {} | {}:{}";
    ...
    std::wstring battery { };
    const auto battery_pc = interfaces::steamutils->get_current_battery_power( );
    if ( battery_pc == -1 ) {
        battery = L"charging";
    } else {
        battery = std::to_wstring( battery_pc ) + L"%";
    }

    const auto watermark_text = std::format( watermark_fmt, name, battery, compile_time, systime.wHour, systime.wMinute );

    const Vector watermark_text_size = renderer::get_text_size( watermark_text, fonts::watermark_font );

    const auto watermark_pos = Vector(
        renderer::get_screen_size( ).x - margin.x - pad.x * 2 - watermark_text_size.x,
        margin.x
    );
    ...
    static auto watermark_outline_col = Color( 139, 126, 214, 255 );
    static auto watermark_text_col = Color( 255, 255, 255, 255 );

    renderer::draw_rectangle_filled( watermark_pos, watermark_size, watermark_col );
    renderer::draw_rectangle( watermark_pos, watermark_size, watermark_outline_col );
    renderer::draw_text( watermark_text, watermark_text_pos, watermark_text_col, fonts::watermark_font );
}
